...
def genetic_algorithm(net, scenario, population_size=20, generations=50, target_voltage=1.0):
    ...
    def evaluate_fitness(individual):
        ...
        print(f"S:{scenario}, PF for ind: {individual}:\n")
        try:
            pp.runpp(net_copy)
            ...
            # Assign a very high fitness value to penalize this individual
            tic=float('inf')
            tgc=float('inf')
            fitness = float('inf')  # Infinite fitness value (penalize)
...
def main():
    ...
    pp.runpp(net_base,max_iteration=50)
    ...
    pp.runpp(net_combined,max_iteration=50)
