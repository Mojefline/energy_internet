...
import numpy as np
import pypsa
from pypsa.components import Link
import matplotlib.pyplot as plt
import random

# Define the EnergyRouter as a custom PyPSA component
class EnergyRouter(Link):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.efficiency = kwargs.get('efficiency', 0.95)
        self.v_set = kwargs.get('v_set', 1.0)

def add_routers(network, router_buses):
    for i, bus in enumerate(router_buses):
        to_bus = f"ER_Bus_{i}"
        network.add("Bus", to_bus, v_nom=12.66)
        network.add("EnergyRouter", f"Router_{i}", 
...
def genetic_algorithm(network, n_routers, population_size=10, generations=10):
    ...
    for gen in range(generations):
        ...
        for ind in population:
            ...
            try:
                temp_network.lopf(pyomo=False, keep_files=True)
...
if __name__ == '__main__':
    # Register the EnergyRouter component with PyPSA
    pypsa.components.component_types["EnergyRouter"] = EnergyRouter
    ...
    network.lopf(pyomo=False, keep_files=True)
